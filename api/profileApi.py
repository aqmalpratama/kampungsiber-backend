...
def updateUserProfile(user_id):
    try:
        ...
        connection = mysql.connect()
        cursor = connection.cursor(pymysql.cursors.DictCursor)
        cursor.execute("UPDATE `user_profile` SET `first_name` = %s, `last_name` = %s, `gender` = %s, `date_of_birth` = %s, `phone` = %s, `address` = %s, `city` = %s, `state` = %s, `country` = %s, `postal_code` = %s, `updated_date` = %s WHERE `user_id` = %s", (data['first_name'], data['last_name']))
        connection.commit()
